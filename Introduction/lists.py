...
def sort_last(tuples):
    tuples.sort(key = lambda x: x[-1])  #.sort(key=lambda x:x[-1])
    return tuples
...
def linear_merge(sorted1, sorted2):
    ...
    indexLst1=0;
    indexLst2=0;
    totalCount=len(sorted1)+len(sorted2)
    flag1=False
    flag2=False

    for i in range(totalCount):
        if flag1:
            lst.append(sorted1[indexLst1])
            indexLst1+=1; 
        elif flag2:
            lst.append(sorted2[indexLst2])
            indexLst2+=1; 
        elif sorted1[indexLst1] <= sorted2[indexLst2]:
            lst.append(sorted1[indexLst1])
            if not indexLst1 == len(sorted1)-1: #save not over flow
                indexLst1+=1; 
            else:
                flag2=True
        else:
            lst.append(sorted2[indexLst2])
            if not indexLst2 == len(sorted2)-1: #save not over flow
                indexLst2+=1; 
            else:
                flag1=True
    return lst
